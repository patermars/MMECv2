...
def main():
    ...
    for fold_idx in range(args.k):
        ...
        # Use 2 folds for validation for larger val set
        val_idx1 = (fold_idx - 1) % args.k
        val_idx2 = (fold_idx - 2) % args.k
        val_fold = [min(folds[val_idx1][0], folds[val_idx2][0]), 
                    max(folds[val_idx1][1], folds[val_idx2][1])]
        train_folds = [folds[i] for i in range(args.k) 
                      if i not in [fold_idx, val_idx1, val_idx2]]
